...
// Version 3 API and higher
unsafe fn ui_searchable_combobox(
    ui: *mut c_void,
    id_salt: &str,
    ...
    let vtable = VTABLE.unwrap();
    let id_salt_cstr = CString::new(id_salt).unwrap();

    let values: Vec<i32> = choices.iter().map(|c| c.0).collect();
    let cstrings: Vec<CString> = choices.iter().map(|c| CString::new(c.1).unwrap()).collect();
    let pointers: Vec<*const c_char> = cstrings.iter().map(|c| c.as_ptr()).collect();

    (vtable.gui_ui_searchable_combobox)(
        ui,
        id_salt_cstr.as_ptr(),
        selected_value as *mut i32,
        values.as_ptr(),
        pointers.as_ptr(),
        choices.len()
    )
